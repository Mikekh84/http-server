...
def test_response_ok_parts(request_good):
    ...
    from concurreny import response_ok
...
def test_response_error_bytes(request_bad_proto):
    ...
    from concurreny import response_error
...
def test_parse_request_good(parse_good):
    ...
    from concurreny import parse_request
...
def test_parse_request_bad_method(request_bad_method):
    ...
    from concurreny import parse_request, HTTPErrors
...
def test_resolve_raise_400(bad_uri):
    ...
    from concurreny import resolve_uri, HTTPErrors
    with pytest.raises(HTTPErrors) as e:
        resolve_uri(bad_uri)
...
def test_resolve_uri_is_file(sample_file):
    from concurreny import resolve_uri
